...
export const Card: React.FC<CardProps> = ({ children, className = '', isActive, ...props }) => {
  return (
    <div className={`bg-white rounded-2xl border ${isActive ? 'border-brand-500 ring-2 ring-brand-100' : 'border-slate-200'} shadow-sm hover:shadow-md transition-shadow duration-300 ${className}`} {...props}>
      {children}
    </div>
  );
};
...
export const CardHeader: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ children, className = '', ...props }) => (
  <div className={`px-5 py-4 border-b border-slate-100 flex items-center justify-between ${className}`} {...props}>{children}</div>
);

export const CardTitle: React.FC<React.HTMLAttributes<HTMLHeadingElement>> = ({ children, className = '', ...props }) => (
  <h3 className={`font-bold text-slate-800 text-lg tracking-tight ${className}`} {...props}>{children}</h3>
);
