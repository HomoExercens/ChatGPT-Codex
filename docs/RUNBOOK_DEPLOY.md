...
- (Recommended for public previews/staging) `NEUROLEAGUE_PREVIEW_MODE=true` — disables expensive anonymous work (e.g. HQ creator kit render enqueue)
...
- (Optional) Playtest report JSON: `<base_url>/api/assets/ops/playtest_report_latest.json` (generated by `scripts/playtest_report.py`)
